...
export const idle = async (token: string) => {
  const response = await fetch(`https://api.idle.finance/junior-rates/${contracts[token]}?isRisk=false&order=desc&limit=1`, {
    headers: {
  ...
  const [json] = await response.json() as { idleRate: string }[]
